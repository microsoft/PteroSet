...
def evaluate_fold(
    ...
    if model.is_binary:
        ...
        all_probs = np.exp(all_logits) / np.exp(all_logits).sum(
...
def plot_precision_recall_curves(fold_results: List[Dict], output_path: str):
    ...
    for i, result in enumerate(fold_results):
        ...
        project_name = (
            FOLD_PROJECT_NAMES[i] if i < len(FOLD_PROJECT_NAMES) else f"Fold {i}"
        )
        ax.plot(
            recall,
            precision,
    ...
    ax.set_title(
        "Precision-Recall Curves - Cross-Validation", fontsize=16, fontweight="bold"
    )
...
def plot_qualitative_results(
    ...
    project_name = (
        FOLD_PROJECT_NAMES[fold_index] if fold_index < len(FOLD_PROJECT_NAMES) else ""
    )
    ...
    freq_tick_labels = [
        f"{int(f / 1000)}k" if f >= 1000 else str(int(f)) for f in freq_ticks_hz
    ]
        ...
        if len(indices) == 0:
            ...
            if csv_row is not None:
                ...
                try:
                    ...
                    print(
                        f"  Warning: could not save audio for fold {fold_index} {cat}: {e}"
                    )
    ...
    fig.suptitle("Qualitative Examples", fontsize=15, fontweight="bold")
    plt.tight_layout()
    ...
    for metric_name, metric_label in zip(metric_names, metric_labels):
        if metric_name in df.columns:
            ...
            if len(values) > 0:
                ...
                max_val = values.max()
                print(
                    f"{metric_label:<20} {mean_val:<12.4f} {std_val:<12.4f} {min_val:<12.4f} {max_val:<12.4f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--config", type=str, default="data/config.yaml", help="Path to config file"
    )
    parser.add_argument(
        "--output_dir", type=str, default="outputs", help="Directory to save plots"
    )
    parser.add_argument(
        "--audio_dir",
        type=str,
        default="data/audios_48khz",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="DataLoader workers (overrides config). Use 0 for small /dev/shm.",
    )
